...
impl Computer {
    pub fn new(size: UVec2, device: &wgpu::Device) -> Computer {
    ...
    pub fn run(&self, device: &wgpu::Device, queue: &wgpu::Queue, mandelbot_params: &MandelbrotParams) -> &wgpu::Texture {
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });

        let m_params_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
